...
The downloaded CSV file should contain at least the following column names:
  ...
  Disposition: The label of the TCE as planet candidate (1) or false positive (0)
...
The TESS data (lightcurve .fits files) is assumed to reside in a directory with the same structure as
...
following light curve representations:
  global_view: Vector of length 201; the Global View of the TCE.
  local_view: Vector of length 61; the Local View of the TCE.
"""
...
def _process_tce(tce, invert):
  ...
  time, flux = preprocess.read_and_process_light_curve(tce.tic_id, FLAGS.tess_data_dir, invert, sector=tce.Sectors, is_multi=tce.is_multi)
...
def _process_file_shard(tce_table, file_name):
  ...
  with tf.io.TFRecordWriter(file_name) as writer:
    ...
    for _, tce in tce_table.iterrows():
        # Skips light curves with no points in given time range
      try:
        example = _process_tce(tce, invert=False)
      ...
      try:
        example = _process_tce(tce, invert=True)
      except (IOError, preprocess.EmptyLightCurveError, SparseLightCurveError):
        num_skipped += 1
        continue
      if example is not None:
...
def main(argv):
  ...
  boundaries = np.linspace(0, len(train_tces),
                           FLAGS.num_train_shards + 1).astype(int)
  for i in range(FLAGS.num_train_shards):
    start = boundaries[i]
    end = boundaries[i + 1]
